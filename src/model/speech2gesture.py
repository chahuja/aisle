...
class Speech2Gesture2_D(nn.Module):
  '''
  Baseline: http://people.eecs.berkeley.edu/~shiry/projects/speech2gesture/
  ...
  def __init__(self, in_channels=104, out_channels=64,  n_downsampling=2, p=0, groups=1, **kwargs):
    ...
    self.conv4 = ConvNormRelu(out_channels*ch_mul_new, out_channels*ch_mul_new,
                              type='1d', leaky=True, p=p, groups=groups)
    self.conv5 = ConvNormRelu(out_channels*ch_mul_new, out_channels*ch_mul_new,
                              type='1d', leaky=True, p=p, groups=groups)

    self.logits = nn.Conv1d(out_channels*ch_mul_new*groups, 1*groups, kernel_size=4, stride=1, groups=groups)
...
class Speech2Gesture3_D(nn.Module):
  '''
  Baseline: http://people.eecs.berkeley.edu/~shiry/projects/speech2gesture/
  ...
  def __init__(self, in_channels=104, out_channels=64,  n_downsampling=2, p=0, groups=1, **kwargs):
    ...
    self.conv1 = nn.Sequential(torch.nn.Conv1d(in_channels*groups, out_channels*groups, 4, 2, padding=1, groups=groups),
                               torch.nn.LeakyReLU(negative_slope=0.2))
    ch_mul_new = min(2**(n_downsampling-1), 8)
    self.conv3 = ConvNormRelu(out_channels, out_channels*ch_mul_new,
                              type='1d', leaky=True, downsample=True, p=p, groups=groups)
